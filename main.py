...
# ==========================================
# 1. CONFIGURACIÓN Y CONEXIÓN DIRECTA A SHANE
# ==========================================
SHANE_URL = os.environ.get("SHANE_URL", "http://localhost:8000")
DERIV_APP_ID = os.getenv("DERIV_APP_ID", "1089")
DERIV_WS_URL = f"wss://ws.derivws.com/websockets/v3?app_id={DERIV_APP_ID}"

def enviar_latido_a_shane():
    """Envía el pulso de 60s a Shane para mantenerlo despierto en Render."""
    try:
        url = f"{SHANE_URL}/api/heartbeat"
        requests.post(url, json={"origen": "MotorPrincipal", "timestamp": time.time()}, timeout=5)
    except Exception:
        pass

def reportar_evento_a_shane(evento: str, detalles: str):
    """
    Envía la alerta al servidor web de Shane.
    Shane procesa la información y la entrega a Telegram.
    """
    try:
        url = f"{SHANE_URL}/api/alerta_motor"
        payload = {"evento": evento, "detalles": detalles}
        requests.post(url, json=payload, timeout=5)
        logging.info(f"📢 [Motor]: Alerta '{evento}' enviada a Shane.")
    except Exception as e:
        logging.error(f"❌ [Motor]: Error enviando evento a Shane: {e}")

# ==========================================
# 2. SISTEMA FRACTAL BIDIRECCIONAL - JUAN TRECH
# ==========================================
class MotorFractalJuanTrech:
    """
    SISTEMA FRACTAL BIDIRECCIONAL - JUAN TRECH
    Regla Maestra:
    - Alcista: Sube el cerro y baja (hace la contra) a buscar la EMA 15
    - Bajista: Va en picada abajo y sube (hace la contra) a buscar la EMA 15
    """
    def __init__(self):
        ...
        self.alarma_enviada = {activo: {t: False for t in self.temporalidades} for activo in self.activos}

    def calcular_ema(self, precios, periodo):
        """Cálculo dinámico de la EMA 15"""
    ...
    async def evaluar_fractalidad_completa(self, activo, precio_actual):
        ...
        for t in self.temporalidades:
            ...
            # Cálculo de la EMA 15 (Punto de bencina)
            ema15 = self.calcular_ema(precios_t, 15)
            if not ema15:
                continue

            # Distancia absoluta en pips a la EMA 15
            distancia_pips = abs(precio_actual - ema15) * 10000
